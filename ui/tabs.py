...
def create_analysis_tabs(full_chain_df, spot_price, metrics, atm_strike, selected_expiry,
            ...
            if iv_chart:
                ...
                with col1:
                    ...
                    st.metric("ATM Call IV",
                             f"{full_chain_df.loc[atm_idx, 'Call IV']:.1f}%")
                with col2:
                    st.metric("Avg Put IV", f"{full_chain_df['Put IV'].mean():.1f}%")
                    st.metric("ATM Put IV",
                             f"{full_chain_df.loc[atm_idx, 'Put IV']:.1f}%")
            else:
    ...
    with tab6:
        if sidebar_config['show_strategy']:
            ...
            # Display strategy payoff and metrics
            payoff_chart, strategy_metrics = charts.create_strategy_payoff(
                full_chain_df, spot_price, strategy, atm_strike, symbol
            )
            ...
            if payoff_chart:
                ...
                cols = st.columns(3)
                cols[0].metric("Max Profit", strategy_metrics.get("Max Profit", "N/A"))
                cols[1].metric("Max Loss", strategy_metrics.get("Max Loss", "N/A"))
                cols[2].metric("Net Premium", strategy_metrics.get("Net Premium", "N/A"))
                
                st.info(f"**Breakeven Points:** {strategy_metrics.get('Breakevens', 'N/A')}")
                st.markdown(f"**Legs:** {strategy_metrics.get('Legs', 'N/A')}")
            else:
                st.warning("Could not generate payoff chart. This may be due to missing options data for the required strikes.")
